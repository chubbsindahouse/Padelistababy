...
export function LeaderboardClient({ entries, seasons, activeSeason }: Props) {
  ...
  const [selectedSeason, setSelectedSeason] = useState<Season | null>(activeSeason);
  const [pastEntries, setPastEntries]     = useState<LeaderboardEntry[] | null>(null);
  const [loadingPast, setLoadingPast]     = useState(false);

  const isActiveSeason = selectedSeason?.id === activeSeason?.id;

  async function switchSeason(season: Season) {
    setSeasonOpen(false);
    ...
    setLoadingPast(true);
    try {
      const res  = await fetch(`/api/leaderboard?season_id=${season.id}`);
      const data = await res.json() as PastEntry[];
      const mapped: LeaderboardEntry[] = data.map((s) => ({
        rank:          s.final_rank,
        profile:       { ...s.profiles, id: s.profiles.id, elo_rating: s.final_elo, total_points: s.final_points, created_at: "", updated_at: "" },
        matchWins:     s.match_wins,
        matchesPlayed: s.match_wins + s.match_losses,
        winRate:       (s.match_wins + s.match_losses) > 0 ? s.match_wins / (s.match_wins + s.match_losses) : 0,
      }));
      setPastEntries(mapped);
    } catch {
      // fall back to current entries silently
    } finally {
  ...
  return (
    <div className="pt-8 pb-4">
      ...
      {seasons.length > 1 && (
        <div className="px-4 mb-4 relative">
          ...
          >
            ...
            <span className="flex-1 text-left">{selectedSeason?.name ?? "Select Season"}</span>
            {!isActiveSeason && (
            ...
            <ChevronDown size={14} className={cn("text-cyan-400 transition-transform", seasonOpen && "rotate-180")} />
          </button>
              ...
              <div className="absolute left-4 right-4 top-full mt-1.5 z-20 glass-card rounded-xl border border-white/10 overflow-hidden shadow-xl">
                {seasons.map((s) => (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center gap-2.5 px-3.5 py-2.5 text-sm text-left transition-colors border-b border-white/[0.06] last:border-0",
                  ...
                  >
                    ...
                    {selectedSeason?.id === s.id && <Check size={13} className="text-cyan-400 shrink-0" />}
                  </button>
                ))}
              </div>
            </>
      ...
      {/* Sort toggle — only for active season (past seasons sort by final snapshot) */}
